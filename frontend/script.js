...
let savedMetrics = JSON.parse(localStorage.getItem('metrics') || '{"per_phase":[], "totals":{"total_response_time":0, "total_tokens":0, "avg_response_time":0}}');
...
async function designTests() {
    ...
    showLoadingTestSuite(`📋 Designing ${testCount} test cases... (This may take 30-60 seconds)`);
...
function updateMetrics() {
    fetch(`${API_BASE}/api/metrics`)
        .then(r => {
            if (!r.ok) {
                console.warn('Metrics endpoint returned:', r.status, '- using cached metrics');
                return null;
            }
            return r.json();
        })
...
function renderEditableTests() {
    ...
    container.innerHTML = editingTests.map((test, index) => `
        <div class="editable-test" style="background: white; border: 1px solid #e0e0e0; border-radius: 8px; padding: 15px; margin-bottom: 12px;">
            ...
            <div style="margin-bottom: 10px;">
                ...
                <textarea onchange="updateTestField(${index}, 'description', this.value)"
                    style="width: 100%; font-size: 12px; border: 1px solid #e0e0e0; border-radius: 4px; padding: 6px; min-height: 50px;">${escapeHtml(test.description)}</textarea>
            </div>
...
function updateTestSteps(index, value) {
    if (editingTests[index]) {
        editingTests[index].steps = value.split('\\n').filter(s => s.trim().length > 0);
    }
}
...
async function sendChatMessage() {
    ...
    try {
        ...
        if (result.success) {
            ...
            if (result.metrics) {
                const existingMetrics = JSON.parse(localStorage.getItem('metrics') || '{"per_phase": [], "totals": {}}');
            ...
            if (result.data.modified_tests && Array.isArray(result.data.modified_tests)) {
                ...
                if (suiteSelect && currentReviewSuiteId) {
                    const option = suiteSelect.querySelector(`option[value="${currentReviewSuiteId}"]`);
                    if (option) {
                        const suite = savedTestSuites.find(s => s.id === currentReviewSuiteId);
                        if (suite) {
...
function addChatMessage(role, content) {
    ...
    messageDiv.style.cssText = `
        ...
        padding: 12px;
        border-radius: 8px;
        ${role === 'user' ? 'background: #667eea; color: white; margin-left: 20%; text-align: right;' : 'background: #f8f9fa; color: #333; margin-right: 20%;'}
    `;
    
    messageDiv.innerHTML = `
        <div style="font-size: 11px; opacity: 0.7; margin-bottom: 4px;">${role === 'user' ? 'You' : 'AI Assistant'}</div>
...
function updateCodegenPreview() {
    ...
    document.getElementById('codegenDisplay').innerHTML = `
        <div style="color: #a0a0a0;">
            ...
            <p style="font-size: 13px; margin-bottom: 10px;">URL: <span style="color: #4ec9b0;">${escapeHtml(suite.url)}</span></p>
        ...
        if (result.success) {
            ...
            document.getElementById('codegenDisplay').innerHTML = `
                <div style="position: relative;">
                    ...
                    <pre style="background: #1e1e1e; color: #d4d4d4; padding: 20px; border-radius: 8px; overflow-x: auto; font-family: 'Consolas', 'Monaco', monospace; font-size: 12px; line-height: 1.5; margin: 0;"><code>${escapeHtml(generatedCode)}</code></pre>
                </div>
            `;
...
function downloadCode() {
    ...
    const suite = savedTestSuites.find(s => s.id === currentCodegenSuiteId);
    const filename = `test_${suite ? suite.name.replace(/\s+/g, '_').toLowerCase() : 'suite'}.py`;
